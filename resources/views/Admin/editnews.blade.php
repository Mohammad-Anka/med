...
<div class="container h-100 d-flex justify-content-center align-items-center">
    <div class="row w-100">
      ...
      @endif
      <input id="subjectold" type="hidden" value="{{ html_entity_decode( $news->subject)}}" />
      <form method="POST" action="{{route('updateNews',[$news->id])}}" enctype="multipart/form-data">
        ...
        <div class="input-group mb-3">
            ...
            <label class="custom-file-label" for="choose-picture">Please choose a picture</label>
            @error('photo')
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="title" aria-describedby="emailHelp" placeholder="Please fill title" value="{{$news->title}}" required>
          @error('title')
          <small class="form-text text-danger"> {{$message}}</small>
          @enderror
        </div>
        <input type="hidden" id='subject' value="{{$news->subject}}">
        <label for="exampleInputEmail1">Please fill subject</label>
        <div id="subjectid">
